...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	// JTKJ: HERE TURN THE MSB AND LSB INTO A SIGNED 16-BIT VALUE
	// AND STORE IT INTO GIVEN VARIABLE data[7]
	int16_t accel_x = (rawData[0] << 8 | rawData[1]);
	data[0] = accel_x;
	int16_t accel_y = (rawData[2] << 8 | rawData[3]);
	data[1] = accel_y;
	int16_t accel_z = (rawData[4] << 8 | rawData[5]);
	data[2] = accel_z;
	
	int16_t gyro_x = (rawData[8] << 8 | rawData[9]);
	data[4] = gyro_x;
	int16_t gyro_y = (rawData[10] << 8 | rawData[11]);
	data[5] = gyro_y;
	int16_t gyro_z = (rawData[12] << 8 | rawData[13]);
	data[6] = gyro_z;
