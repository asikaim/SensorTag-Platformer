...
static uint32_t reg_to20bit(uint8_t xlsb, uint8_t lsb, uint8_t msb) {
...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
        ...
        uint32_t lampo = reg_to20bit(rxBuffer[5], rxBuffer[4], rxBuffer[3]);
        uint32_t paine = reg_to20bit(rxBuffer[2], rxBuffer[1], rxBuffer[0]);
		...
		*temp = bmp280_convert_temp(lampo);
